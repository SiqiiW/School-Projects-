...
int nru()
{
    ...
    for (int i = 0; i < mem_size; i++){
        int currentPage = mem[i];
        if (page_table[currentPage].R *2 + page_table[currentPage].M < minCount){
            minCount = page_table[currentPage].R *2 + page_table[currentPage].M;
            victim = i;
        }
    }
...
int aging()
{
    ...
    for (int i = 0; i < mem_size; i++){
        int currentPage = mem[i];
        if (page_table[currentPage].counter < minCount){
            minCount = page_table[currentPage].counter;
            victim = i;
        }
    }
...
{
    // handle the page accessed
    page_table[page].R = 1;
    page_table[page].counter = page_table[page].counter >> 1;  // right shift
    page_table[page].counter = page_table[page].counter + 128; // add R

    // handle the rest
    for (int i = 0; i < mem_size; i++){
        int currentPage = mem[i];
        if (currentPage != page){
            page_table[currentPage].counter = page_table[currentPage].counter >> 1;  // right shift
            if (page_table[currentPage].R == 1){
                page_table[currentPage].counter = page_table[currentPage].counter + 128; // add R
            }
            page_table[currentPage].R = 0; // clear R
        }
    }
}
/* **************** End of your code *************** */
...
int insert(int page)
{
   ...
   for(i = 0; i < mem_size; i++)
     ...
     if(mem[i] == page) // already exist
		 return i;  
     else if(!mem[i]) // first empty one
     {
        mem[i] = page; 
		return i;
     }
